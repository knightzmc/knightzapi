...
public class MenuSerializer extends TypeAdapter<Menu> {
    ...
    public void write(JsonWriter out, Menu menu) throws IOException {
        out.beginObject();
        out.name("inventory");
        KnightzAPI.gson.getAdapter(Inventory.class).write(out, menu.getInv());
    ...
    public Menu read(JsonReader in) throws IOException {
        ...
        while (in.hasNext()) {
            switch (in.nextName()) {
                ...
                case "children":
                    in.beginArray();
                    Menu[] menus = KnightzAPI.gson.fromJson(in, new TypeToken<Menu[]>() {}.getRawType());
